...
pub mod noshot {
    ...
    pub fn resolve_bet(ctx: Context<ResolveBet>, winning_side: BetSide) -> Result<()> {
        ...
        msg!("Total pool: {} USDC", group.total_yes + group.total_no);
        
        Ok(())
    }

    /// Claim winnings - automatic disbursement
    pub fn claim_winnings(ctx: Context<ClaimWinnings>) -> Result<()> {
        ...
        // Proportional distribution: (winner's bet / winner pool) * total pool
        let winner_share = (member.amount_bet as u128)
            .checked_mul(total_pool as u128)
            .unwrap()
            .checked_div(winner_pool as u128)
            .unwrap() as u64;
        
        msg!("Claiming {} USDC for {:?}", winner_share, member.wallet);
        ...
        token::transfer(cpi_ctx, winner_share)?;
